...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall Tryaksh and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Tryaksh")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g tryaksh",
      pnpm: "pnpm uninstall -g tryaksh",
      bun: "bun remove -g tryaksh",
      yarn: "yarn global remove tryaksh",
      brew: "brew uninstall tryaksh",
      choco: "choco uninstall tryaksh",
      scoop: "scoop uninstall tryaksh",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Tryaksh!")
}
