// CLI entry point for `tryaksh run`.
//
// Handles three modes:
...
//      Tryaksh server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run Tryaksh with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running Tryaksh server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to TRYAKSH_SERVER_USERNAME, OPENCODE_SERVER_USERNAME, or 'tryaksh')",
      })
      .option("dir", {
        type: "string",
